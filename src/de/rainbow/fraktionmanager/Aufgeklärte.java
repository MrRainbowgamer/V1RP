...
public class Aufgeklärte implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if(commandSender instanceof Player) {
            ...
            if(p.hasPermission("frak.managemant.admin") || sp.getCity().equalsIgnoreCase("Aufgeklärte")) {
                if(args.length == 0) {
                    p.sendMessage("/aufgeklärte setleader [name]");
                    p.sendMessage("/aufgeklärte invite [name]");
                    p.sendMessage("/aufgeklärte kick [name]");
                    p.sendMessage("/aufgeklärte promote [name]");
                    p.sendMessage("/aufgeklärte demote [name]");
                    ...
                    if(args[0].equalsIgnoreCase("setleader")) {
                        ...
                        if(sp.getCityRang() > 3 || p.hasPermission("frak.managemant.admin")) {
                            Player p2 = Bukkit.getServer().getPlayer(args[1]);
                            Roleplay.invites.put(p2, "Aufgeklärte");
                            p.sendMessage(Roleplay.prefix + "Du hast " + p2.getName() + " eine einladunge geschickt");
                            p2.sendMessage(Roleplay.prefix + "Du hat eine Einladung bekommen [/acceptinvite aufgeklärte]");
